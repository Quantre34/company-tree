...
export function Canvas({ onAddChild, onSelectionEmpty, svgRef }: Props) {
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Escape' && dragState) {
      ...
      const wasDetach = !!state.historyByWorkspace[state.activeWorkspaceId]?.past.length;
  ...
  return (
    ...
    >
      ...
      <div className="canvas-controls">
        <button onClick={() => zoomBy(1 / 1.2)} title={t('canvas.zoomOut')}>−</button>
        <button onClick={fitToView} title={t('canvas.fitToScreenTitle')}>{t('canvas.fitToScreen')}</button>
        <button onClick={() => zoomBy(1.2)} title={t('canvas.zoomIn')}>+</button>
        <button onClick={() => setView({ scale: 1, tx: 40, ty: 40 })} title={t('canvas.actualSize')}>100%</button>
      </div>
    </div>
  );
}
