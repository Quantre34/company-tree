...
export async function exportXlsx(doc: OrgDoc) {
  ...
  // ---- Sheet 1: Şema (visual tree) ----
  const wsTree = wb.addWorksheet('Şema', {
  ...
  try {
    ...
    metaCell.value = `${doc.meta.orgName} · ${new Date(doc.meta.updatedAt).toLocaleDateString('tr-TR')}`;
    ...
    wsTree.getCell(1, 1).value = 'Şema görseli üretilemedi.';
  }

  // ---- Sheet 2: Organizasyon (flat list) ----
  const ws = wb.addWorksheet('Organizasyon');
  ws.columns = [
    { header: 'Seviye', key: 'level', width: 8 },
    { header: 'Üst Birim', key: 'parent', width: 30 },
    { header: 'Birim / Pozisyon', key: 'unit', width: 34 },
    { header: 'Alt Başlık', key: 'subtitle', width: 26 },
    { header: 'Kişi', key: 'person', width: 26 },
    { header: 'Kişi Unvanı', key: 'personRole', width: 22 },
    { header: 'Açıklama', key: 'description', width: 38 },
    { header: 'Yol', key: 'path', width: 60 },
  ];
  ...
  ws2.addRow(['Birim sayısı', totalUnits]);
  ws2.addRow(['Kişi sayısı', totalPeople]);
  ws2.addRow(['En derin seviye', maxDepth]);
  ws2.addRow([]);
  ws2.addRow(['Birim', 'Kişi Sayısı']);
