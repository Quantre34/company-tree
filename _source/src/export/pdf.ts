...
import { translate, useLangStore, dateLocale } from '../i18n/langStore';
...
export async function renderPdf(doc: OrgDoc, opts: PdfOptions = {}): Promise<jsPDF> {
  ...
  // Full-page fill so the page background matches the tree's own canvas
  // color instead of showing jsPDF's default white around it.
  const bgHex = (doc.meta.theme.canvasBg || '#FFFFFF').replace('#', '');
  ...
  const dateStr = parsedDate && !isNaN(parsedDate.getTime())
    ? parsedDate.toLocaleDateString(dateLocale(lang)) : '';
  ...
  pdf.text(translate('pdf.footerConfidential'), marginX, ph - 18);
  pdf.text(`CompanyTree · ${new Date().toLocaleString(dateLocale(lang))}`, pw - marginX, ph - 18, { align: 'right' });
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        if (!g) return reject(new Error(translate('error.canvas2dUnavailable')));
        g.drawImage(img, 0, 0);
        resolve({
    ...
    img.onerror = () => reject(new Error(translate('error.imageLoadFailed', { src })));
    img.src = src;
  });
}
